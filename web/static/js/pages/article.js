/* ============================================================
   Biz Studio — Trang "Bài viết → Video"
   Đăng ký App.pages['article']. Load sau api.js/ui.js/app.js.
   ============================================================ */
(function () {
  'use strict';
  ...
  var STEPS = ['Tạo video', 'Kịch bản', 'Media', 'Cấu hình'];
  ...
  App.pages['article'] = {
    ...
    subtitle: 'Biến bài viết thành video có giọng đọc, media và phụ đề tự động',
    render: render
  };

  function render(root) {
    var st = {
      step: 1, title: '', content: '', count: 8, scenes: [], jobHandler: null,
      cfg: { aspect: '16:9', voice: '', engine: '', theme: 'Tin nhanh', bgmPath: '', bgmVolume: 25, burnSub: false }
    };
    ...
    var leftCard = UI.card({
      ...
      desc: 'Nhập bài viết, LLM sẽ tách thành các cảnh có lời đọc và từ khóa media.',
      body: [
        UI.field('Tiêu đề', titleInput),
        UI.field('Nội dung bài viết', contentTA),
        charCount,
    ...
    async function prepare() {
      if (!st.content.trim()) { UI.toast('Vui lòng nhập nội dung bài viết trước.', 'error'); return; }
      ...
      try {
        ...
        st.scenes = scenes.map(function (s) {
          return {
            title: s.title || '', voiceText: s.voiceText || '',
        ...
        renderScenes();
        setStep(3);
        UI.toast('Đã tạo ' + st.scenes.length + ' cảnh từ bài viết.');
      } catch (err) {
    ...
    var rightCard = h('div', { class: 'card' },
      ...
      h('div', { class: 'mt-8' }, UI.btn('+ Thêm cảnh', {
        ...
        onclick: function () {
          st.scenes.push({ title: 'Cảnh ' + (st.scenes.length + 1), voiceText: '', mediaKeyword: '', duration: 6 });
          renderScenes();
        }
      })));
    ...
    function renderScenes() {
      ...
      st.scenes.forEach(function (sc, i) {
        tbody.appendChild(h('tr', null,
          ...
          h('td', null, cellInput(sc, 'mediaKeyword', { placeholder: 'từ khóa' })),
      ...
      tableHost.appendChild(h('table', { class: 'table' },
        h('thead', null, h('tr', null,
          ...
          h('th', { style: { width: '20%' } }, 'Tiêu đề'),
          h('th', null, 'Lời đọc'),
          h('th', { style: { width: '17%' } }, 'Từ khóa media'),
          h('th', { style: { width: '68px' } }, 'Giây'),
          h('th', { style: { width: '44px' } }, ''))),
        tbody));
    }

    // ---------- Dưới: cấu hình & dựng video ----------
    ...
    var renderBtn = UI.btn('🎬 Tạo video', { variant: 'primary', large: true, onclick: startRender });
    var cfgCard = UI.card({
    ...
    async function startRender() {
      ...
      var body = {
        scenes: st.scenes.map(function (s) {
          return { title: s.title, voiceText: s.voiceText, mediaKeyword: s.mediaKeyword, duration: Number(s.duration) || 6 };
        }),
        config: {
    ...
    function trackJob(job) {
      ...
      resultHost.appendChild(h('div', null,
        h('div', { class: 'row' }, UI.spinner(), h('b', null, 'Đang dựng video từng cảnh…')),
